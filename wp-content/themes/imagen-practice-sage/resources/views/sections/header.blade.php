<header class="bg-primary bg-white fixed left-0 top-0 w-full z-50" id="primarynav">
  ...
  <div class="bg-secondary p-2 hidden lg:block">
    <div class="xl:container mx-auto px-6">
      <div class="flex flex-row-reverse items-center desktop-nav">
        <div class="relative">{!! get_search_form(true) !!}</div>

        @if (has_nav_menu('secondary_navigation'))
          <nav class="nav-secondary flex flex-row-reverse text-white uppercase text-xs font-medium" aria-label="{{ wp_get_nav_menu_name('secondary_navigation') }}">
  ...
  <div class="xl:container mx-auto px-6">
    <div class="flex justify-between items-center py-6 lg:justify-start lg:space-x-10">
      ...
      <div class="-mr-2 -my-2 lg:hidden flex">
        ...
        <a href="tel:{{ the_field('practice_phone', 'option') }}" type="button" class="relative px-2 items-center justify-center text-white text-center" aria-expanded="false" style="height: 40px; width: 60px;">
        ...
        @php($obLink = get_field('schedule_form_outbound_link', 'option'))
        @if($obLink)
        <a href="{{ $obLink }}" type="button" class="relative px-2 items-center justify-center text-white text-center" style="height: 40px; width: 60px;">
      ...
      <div class="hidden lg:flex space-x-10 desktop-nav">
        @if (has_nav_menu('primary_navigation'))
          <nav class="nav-primary uppercase text-xs font-medium" aria-label="{{ wp_get_nav_menu_name('primary_navigation') }}">
      ...
      <div class="hidden lg:flex items-center justify-end lg:flex-1 lg:w-0">
        <a href="tel:{{ the_field('practice_phone', 'option') }}" class="whitespace-nowrap text-xs font-medium hidden xl:inline-block">
          ...
          {{ the_field('practice_phone', 'option') }} </a>

        <a href="tel:{{ the_field('practice_phone', 'option') }}" type="button" class="xl:hidden relative items-center justify-center text-primary text-center" aria-expanded="false" style="height: 40px; width: 60px;">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512" height="24" class="inline mb-2"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path class="fill-primary" d="M304 0h-224c-35.35 0-64 28.65-64 64v384c0 35.35 28.65 64 64 64h224c35.35 0 64-28.65 64-64V64C368 28.65 339.3 0 304 0zM192 480c-17.75 0-32-14.25-32-32s14.25-32 32-32s32 14.25 32 32S209.8 480 192 480zM304 64v320h-224V64H304z"/></svg>
          <span class="block uppercase" style="font-size: 9px;">Call</span>
        </a>
        @php($obLink = get_field('schedule_form_outbound_link', 'option'))
        @if($obLink)
        <a href="{{ $obLink }}" type="button" class="ml-4 whitespace-nowrap inline-flex items-center justify-center px-4 py-2 border border-transparent rounded-sm shadow-sm text-xs font-medium text-white bg-primary text-left">
  ...
  <div class="hidden mobile-menu bg-primary">
    ...
    @if (has_nav_menu('primary_navigation'))
      <nav class="nav-primary text-white uppercase font-medium text-center" aria-label="{{ wp_get_nav_menu_name('primary_navigation') }}">
        {!! wp_nav_menu(['theme_location' => 'primary_navigation', 'menu_class' => 'nav', 'echo' => false]) !!}
      </nav>
    @endif

    @if (has_nav_menu('secondary_navigation'))
      <nav class="nav-secondary text-white uppercase text-center" aria-label="{{ wp_get_nav_menu_name('secondary_navigation') }}">
        {!! wp_nav_menu(['theme_location' => 'secondary_navigation', 'menu_class' => 'nav', 'echo' => false]) !!}
      </nav>
    @endif
  </div>
  ...
  <div class="hidden mobile-search bg-primary">
    <div class="relative p-5">{!! get_search_form(true) !!}</div>
  </div>
</header>