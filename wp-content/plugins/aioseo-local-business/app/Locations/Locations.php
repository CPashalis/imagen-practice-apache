...
class Locations {
	...
	public function getLocation( $postId ) {
		...
		return apply_filters( 'aioseo_local_business_get_location', json_decode( $post->local_seo ), $postId );
	}
	...
	public function outputLocationCategory( $termId, $instance = [] ) {
		...
		if ( empty( $locations ) ) {
			return sprintf( __( 'No %s found', 'aioseo-local-business' ), aioseoLocalBusiness()->postType->getPluralLabel() );
		}
	...
	public function outputOpeningHours( $postId, $instance = [] ) {
		...
		if ( empty( $openingHoursData->show ) ) {
			if ( aioseo()->blocks->isGBEditor() ) {
				return sprintf(
					// Translators: 1 - The location post title or empty.
					__( 'Your Opening Hours settings are disabled%1$s. Please enable them to use the Opening Hours block.', 'aioseo-local-business' ),
					$postId ? ' for "' . get_the_title( $postId ) . '"' : ''
				);
			}
	...
	public function outputLocationCategories( $instance = [] ) {
		...
		if ( empty( $categories ) ) {
			return sprintf( __( 'No %1$s found', 'aioseo-local-business' ), aioseoLocalBusiness()->taxonomy->getPluralLabel() );
		}
