...
if(	$currentform[0]->template_type=="widget"){
	$iswidget=true;
?>
<div class="wprevpro_t11_outer_div_widget w3_wprs-row wprevprodiv">
<?php
	} else {
	$iswidget=false;
?>
<div class="wprevpro_t11_outer_div w3_wprs-row wprevprodiv">
<?php
}
	}
...
foreach ( $rowarray[$x] as $review ) 
{
	//fix for slickslider more than one row loading on ajax
		...
		//star number --------------------------------------
		$starfile_burl_logo_array = $templateclass->wprevpro_get_star_logo_burl($review,$imgs_url,$currentform[0],"t11",$template_misc_array);
	...
	if($currentform[0]->display_num>0){
		...
		$companyhtml = $templateclass->wprevpro_get_companyhtml($review,$template_misc_array,"t11");
		...
		if($userpic==""){
			...
			$userpichtml = $profilelink['start'].'<img loading="lazy" src="'.$userpic.'" alt="'.$altname.' Avatar" class="wpproslider_t11_IMG_2" />'.$profilelink['end'];
			if(isset($template_misc_array['avataropt'])){
				if($template_misc_array['avataropt']=='hide'){
					...
					$userpichtml = $profilelink['start'].'<img loading="lazy" src="'.$tempuserpic.'" alt="'.$altname.' Avatar" class="wpproslider_t11_IMG_2 wprevpro_avatarimg" />'.$profilelink['end'];
		...
		if(isset($template_misc_array['showtitle'])){
			if(isset($review->review_title)){
				if($template_misc_array['showtitle']=="hide"){
					...
					$title = '<span class="wprevpro_t11_DIV_title">'.stripslashes(strip_tags($review->review_title)).' </span>';
				}
			}
		}
		...
		$tempmasonrydiv =' wprevpro_t11_DIV_1'.$tempwidget.' w3_wprs-col l'.$perrow;
		...
		if(	$currentform[0]->display_masonry=="yes" && $currentform[0]->createslider != "sli"){
			$tempmasonrydiv =$masonryclass_item.' wprevpro_t11_DIV_1'.$tempwidget.' ';
		}
		...
		//woocommerce $miscpichtmlarray['imagehtml'], $miscpichtmlarray['titlehtml']
		$miscpichtmlarray = $templateclass->wprevpro_get_miscpichtml_t11($review,$currentform[0]);
...
<div class="wpproslider_t11_DIV_1a">
	<div class="wpproslider_t11_DIV_2 wprev_preview_bg1_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?> wprev_preview_bradius_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?> ">
	<?php echo $miscpichtmlarray['imagehtml']; ?>
	<div class="wpproslider_t11_DIV_2_top">
		<div class="wpproslider_t11_DIV_3">
			<div class="t11displayname wpproslider_t11_STRONG_5 wprev_preview_tcolor2_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>"><?php echo $tempreviewername; ?><span class="wpproslider_t11_SPAN_6 wprev_preview_tcolor2_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?> wprev_showdate_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>"><?php echo $datestring; ?></span></div>
			<?php echo $companyhtml;?>
			<div class="wpproslider_t11_star_DIV"><span class="wprevpro_star_imgs_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>"><?php echo $starhtml; ?><?php echo $verifiedstarhtmlarray[0]; ?></span></div>
			<div class="indrevdiv wpproslider_t11_DIV_4">
				<p class="wpproslider_t11_P_4 wprev_preview_tcolor1_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>">
				<?php echo $title; ?><?php echo stripslashes($reviewtext); ?><?php echo $verifiedstarhtmlarray[1]; ?>
				</p>
				<?php echo $miscpichtmlarray['titlehtml']; ?>
				<?php echo $media; ?>
	...
	<div class="wpproslider_t11_DIV_3_logo"><?php echo $logo; ?></div>
	</div>
