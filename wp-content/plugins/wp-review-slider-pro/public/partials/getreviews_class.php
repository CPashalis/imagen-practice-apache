	...
	public function wppro_queryreviews($currentform,$startoffset=0,$totaltoget=0,$notinstring='',$shortcodepageid='',$shortcodelang='',$cpostid='', $textsearch='', $textsort='', $textrating='', $textlang='',$shortcodetag='',$sc_strhasone='',$sc_strhasall='',$sc_strnot=''){
		...
		if($textsearch!=''){
			if($template_misc_array['header_tag_search']=='tags'){
				...
				$textsearchquery = "AND (reviewer_name LIKE '%%".$textsearch."%%' or review_text LIKE '%%".$textsearch."%%' or review_title LIKE '%%".$textsearch."%%' or tags LIKE '%%".$textsearch."%%' or type LIKE '%%".$textsearch."%%')";
			} else {
			$textsearchquery = "AND (reviewer_name LIKE '%%".$textsearch."%%' or review_text LIKE '%%".$textsearch."%%' or review_title LIKE '%%".$textsearch."%%' or type LIKE '%%".$textsearch."%%')";
			}
		}
...
//print_r($totalreviews);
	...
	public function wppro_getloadmorebtnhtml($currentform,$iswidget,$makingslideshow, $notinstring='',$shortcodepageid='',$shortcodelang='',$cpostid='',$totalcount='',$shortcodetag='' ){
		...
		if($currentform[0]->load_more=="yes" ){
			...
			if($makingslideshow || $btnorpagenums=='btn'){
				...
				$resultsecho = $resultsecho. '<div '.$hidepagination.' class="wprevpro_load_more_div"><button data-notinstring="'.$notinstring.'" '.$mobileoneperslide.' '.$reviewssameheight.' '.$endlessscroll.' data-callnum="1" data-ismasonry="'.$currentform[0]->display_masonry.'" data-slideshow="'.$ismakingslideshow.'" data-tid="'.$formid.'" data-nrows="'.intval($currentform[0]->display_num_rows).'" data-perrow="'.intval($currentform[0]->display_num).'" data-cpostid="'.$cpostid.'" data-shortcodepageid="'.$shortcodepageid.'" data-shortcodelang="'.$shortcodelang.'" data-shortcodetag="'.$shortcodetag.'" class="wprevpro_load_more_btn brnprevclass" id="wprev_load_more_btn_'.$formid.'" '.$hidebtnhtml.'>'.esc_html( $currentform[0]->load_more_text ).'</button><img loading="lazy" src="'.$loading_img_url.'" class="wprploadmore_loading_image '.$imageclassslideshow.'" alt="Button to load more customer reviews" style="display:none;"></div>';
			}
			if($makingslideshow){
	...
	public function wppro_getgooglesnippet($currentform,$totalcount,$totalavg,$totalreviews=''){
		...
		if($google_snippet_add=="yes" && $totalavg>0){
			...
			$tempsnippethtml = '<script type="application/ld+json">{'.$schemaidtext.'"@context": "http://schema.org/","@type": "'.$google_snippet_type.'","name": "'.sanitize_text_field($google_snippet_name).'","description": "'.sanitize_text_field($google_snippet_desc).'","aggregateRating": {"@type": "AggregateRating","ratingValue": "'.$totalavg.'","'.$ratingcounthtml.'": "'.$totalcount.'","bestRating": "5","worstRating": "1"},"image": "'.esc_url($google_snippet_business_image).'"'.$gsaddress.$gsphone.$gsprice.$prodmoretxt.$irmtext.'}</script>';
	...
	public function wppro_gettemplatestylecode($currentform,$iswidget,$template_misc_array){
		...
		if(is_array($template_misc_array)){
			...
			if(isset($template_misc_array['screensize'])){
				if($template_misc_array['screensize']=='desk'){	//show desktop only
					$misc_style = $misc_style . '@media only screen and (max-width: 600px) {
					  .wprs_unslider, .wprevpro, .wprev_header_txt, .wprev_search_sort_bar {display: none;}
					}';
				}
			...
			if($currentform[0]->style=="2"){
				...
				if(isset($template_misc_array['bcolor']) && $template_misc_array['bcolor']!='' && str_contains($template_misc_array['bcolor'], ',0)')==false ){
			...
			$misc_style = $misc_style . '#wprev-slider-'.$formid.$iswidget.' .wprs_rd_more,.wprs_rd_less{color:'.$template_misc_array['readmcolor'].';}';
			}
