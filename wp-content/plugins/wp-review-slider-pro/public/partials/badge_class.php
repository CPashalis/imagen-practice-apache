...
class badgetools
{
	...
	public function getimgheightwidthhtml($imagePath){
		...
		if ($pos === false) {
			...
			if($sizeInfo !== false &&  $sizeInfo[0]>0 &&  $sizeInfo[1]>0){
				$wh_html = $sizeInfo[3];
			}
		}
		*/
			...
			foreach ($tempsiconarray as $keysi => $valuesi) {
				...
				if (in_array($temptype, $tempsiconarray)){
					...
					if($temptype!='custom' && $temptype!='custom2' && $temptype!='custom3'){
						...
						$logodirloc= dirname(__FILE__).'/imgs/'.$temptype.'_small_icon.png';
						$wh_html = $this->getimgheightwidthhtml($logodirloc);
				
						if($tempsiiconurl!=''){
							$smalliconshtml= $smalliconshtml . '<a href="'.$tempsiiconurl.'" '.$logourllinktargethtml.' '.$followorno.'><img src="'.$imgs_url.$temptype.'_small_icon.png" '.$wh_html.' alt="'.$temptype.' logo" class="wppro_badge'.$stylenum.'_IMG_4"></a>';
						} else {
							$smalliconshtml= $smalliconshtml . '<img src="'.$imgs_url.$temptype.'_small_icon.png" '.$wh_html.' alt="'.$temptype.' logo" class="wppro_badge'.$stylenum.'_IMG_4 wprevsmallbadgeicon">';
						}
					} else {
						if($temptype=='custom') {
	...
	public function gettotalsaverages($template_misc_array='',$currentform='') {
		...
		*/
		...
		$roundplace = 1;
		if(isset($template_misc_array['roundavg']) && $template_misc_array['roundavg']>0){
