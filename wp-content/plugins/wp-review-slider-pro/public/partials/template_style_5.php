...
foreach ( $rowarray[$x] as $review ) 
{
	...
	if($currentform[0]->display_num>0){
		...
		if($userpic==""){
			...
			$userpichtml = $profilelink['start'].'<img loading="lazy" src="'.$userpic.'" alt="'.$altname.' Avatar" class="wpproslider_t5_IMG_2 wprevpro_avatarimg" />'.$profilelink['end'];
		}
		...
		if(isset($template_misc_array['avataropt'])){
			if($template_misc_array['avataropt']=='hide'){
				...
				$userpichtml = $profilelink['start'].'<img loading="lazy" src="'.$tempuserpic.'" alt="'.$altname.' Avatar" class="wpproslider_t5_IMG_2 wprevpro_avatarimg" />'.$profilelink['end'];
