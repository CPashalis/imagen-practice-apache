...
for ($x = 0; $x < count($rowarray); $x++) {
	...
	foreach ( $rowarray[$x] as $review ) 
	{
		...
		$logo =  "<div class='wprevt4_icon_div'>".$starfile_burl_logo_array['logo']."</div>";
		...
		if($userpic==""){
			...
			$userpichtml = '<div class="wpproslider_t4_avatar_div">'.$profilelink['start'].'<img loading="lazy" src="'.$userpic.'" alt="'.$altname.' Avatar" class="wpproslider_t4_IMG_2 wprevpro_avatarimg" />'.$profilelink['end'].'</div>';
			if(isset($template_misc_array['avataropt'])){
				if($template_misc_array['avataropt']=='hide'){
					...
					$userpichtml = '<div class="wpproslider_t4_avatar_div">'.$profilelink['start'].'<img loading="lazy" src="'.$tempuserpic.'" alt="'.$altname.' Avatar" class="wpproslider_t4_IMG_2 wprevpro_avatarimg"/>'.$profilelink['end'].'</div>';
