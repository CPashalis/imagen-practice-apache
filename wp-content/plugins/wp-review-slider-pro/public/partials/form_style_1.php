...
for ($x = 0; $x < count($formfieldsarray); $x++) {
	...
	if($formfieldsarray[$x]['hide_field']==''){
		...
		if($formfieldsarray[$x]['show_label']=='on'){
			$formfieldshtml = $formfieldshtml . '<label for="wprevpro_'.esc_attr($formfieldsarray[$x]['name']).'">'.esc_html($formfieldsarray[$x]['label']).'</label>';
		}
		...
		if($formfieldsarray[$x]['input_type']=='review_rating' || $formfieldsarray[$x]['input_type']=='starrating'){
		...
		} else if($formfieldsarray[$x]['input_type']=='select_page' && $formfieldsarray[$x]['selopts']!=''){	//for select page select_page
			//get comman string of options. opt1, opt2, opt 3,
			$totalloop = 1;
			$optsarray = array();
			$optnamessarray = array();
				...
				$formfieldshtml = $formfieldshtml .'	<select name="wprevpro_'.$formfieldsarray[$x]['name'].'" id="wprevpro_'.$formfieldsarray[$x]['name'].'">';
		...
		//add a hidden input for the label if this is a custom field
