...
{
    //create a one-time cron to run in 5 minutes to make sure everything completed. Repeat if it didn't.
    wp_schedule_single_event( time() + 300, 'wprevpro_rf_getapps_cron_after' );
    ...
    foreach ( $currentforms as $currentform ) {
        if ( $currentform->cron != '' && $currentform->cron_last_ran != '' ) {
            ...
            if ( $currentform->cron_last_ran > $currentform->cron_last_download ) {
                ...
                if ( $currentform->cron_numtimes_checked < 4 && (!isset( $scrapethejob['crawl_status'] ) || $scrapethejob['crawl_status'] == '') ) {
                    ...
                    if ( $scrapethejob['crawl_status'] != 'pending' || $currentform->cron_numtimes_checked > 3 ) {
