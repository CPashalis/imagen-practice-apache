			...
			if($placeidvalue!=''){
				...
				foreach( $response['result']['reviews'] as $item)
				{
					...
					//{
						...
						$slashedusername = addslashes($user_name);
						
						$checkrow2 = $wpdb->get_var( "SELECT id FROM ".$table_name." WHERE reviewer_name = '".$slashedusername."' AND type = 'Google' AND (review_length_char = '".$review_length_char."' OR review_length = '".$review_length."' OR created_time_stamp = '".$item['time']."')" );
		...
		if (is_array($curselrevs)){
			...
			if($filtertext!=""){
				$reviewsrows = $wpdb->get_results("SELECT * FROM ".$table_name."
					WHERE (reviewer_name LIKE '%".$filtertext."%' or review_title LIKE '%".$filtertext."%' or review_text LIKE '%".$filtertext."%' or tags LIKE '%".$filtertext."%' or pageid LIKE '%".$filtertext."%') AND ".$filterratingtext.$filtertypetext.$filterlangtext.$filterpagetext."
...
//--======================= end yelp =======================--//	
	...
	public function wprevpro_download_img_tolocal() {
		...
		foreach ( $currentreviews as $review ) 
		{
			...
			if($isimage && $userpic!=''){
				...
				if($userpic!=''){
					if(!$newfilelowExist && $newfileExist){
	...
	public function wprevpro_download_tripadvisor_showuserreviews_url($currenturl) {
		...
		if ( is_array( $response ) ) {
		  ...
		  $fileurlcontents = $response['body']; // use the content
			...
			echo esc_html__('Error 02: Error finding reviews. Please contact plugin support.', 'wp-review-slider-pro');
			die();
		}
			...
			if($html->find("div[class*=ReviewTitle]", 0)){
				...
				if(isset($urlmatches['1'])){
					...
					$nextbtnlink = "/ShowUserReviews-".$urlmatches['7'];
			...
			//find page name here
					...
					if($pagename==''){
						...
						$pagename = $html->find('h1[data-test-target=top-info-header]',0)->plaintext;
						}
					}
					//=====================
			...
			if(isset($reviewobject5) && $reviewobject5!="" && $nextbtnlink==''){
			...
			//try to find total and average here from first page
			$totalreviews = $this->get_string_between($fileurlcontents, '"reviewSummary":{"count":', '}', ',');
			if($totalreviews==''){
			$totalreviews = $this->get_string_between($fileurlcontents, '"reviewCount":', '}');
			}
			$avgrating = $this->get_string_between($fileurlcontents, '"reviewSummary":{"rating":', ',','}');
			if($avgrating==''){
			$avgrating = $this->get_string_between($fileurlcontents, ',"ratingValue":"', '"');
			}
			if($totalreviews=='' || $totalreviews<1){
				...
				//AggregateRating [ratingValue] => 4.5 [reviewCount] => 1542 
				if(isset($jsonschemaarray['aggregateRating'])){
					if(isset($jsonschemaarray['aggregateRating']['reviewCount'])){
						$totalreviews=$jsonschemaarray['aggregateRating']['reviewCount'];
					}
					if(isset($jsonschemaarray['aggregateRating']['ratingValue'])){
						$avgrating=$jsonschemaarray['aggregateRating']['ratingValue'];
					}
				}
			...
			$response =array("page1"=>$newurl,"page2"=>$page2url,"pagename"=>$pagename,"totalreviews"=>$totalreviews,"avgrating"=>$avgrating);
			$html->clear(); 
			unset($html);
	...
	public function wprevpro_download_tripadvisor_master_perurl($currenturl,$urlnum,$scrapemethod) {
		...
		$tripadvisoroptions = get_option('wprevpro_tripadvisor_settings');
			...
			//if $currenturl does not have ShowUserReviews in title then we need to try and get from page. AttractionProductReview
			$totalreviews ='';
			$avgrating = '';
		
			//https://www.tripadvisor.it/Hotel_Review-g1189834-d601973-Reviews-Astarte_Suites-Akrotiri_Santorini_Cyclades_South_Aegean.html
			if (strpos($currenturl, 'ShowUserReviews') === false && strpos($currenturl, 'AttractionProductReview') === false ) {
			...
			//die();
				...
				if(isset($nexturl) && $nexturl!="" && $totalreviews>5){
				$tripadvisorurl[1] = $nexturl;
				}
				
				//if this activities then use original Url
				//if($vactionrental==true){
				//	$tripadvisorurl[0] = $listedurl;
				//	$tripadvisorurl[1] = '';
				//	unset($tripadvisorurl[1]);
				//}
				...
				if($usephantomsimple==true) {
					...
					//this one scrolls down 
					$tempurlvalue = 'https://phantomjscloud.com/api/browser/v2/ak-2cme5-eqftq-dv73x-nr41t-gkbvs/?request=%7Burl:%22'.$urlvalue.'%22,renderType:%22html%22,scripts:%7BdomReady:%5B%22setInterval%28function%28%29%7Bvar%20lastY=window.scrollY;window.scrollBy%280,50%29;if%28lastY===window.scrollY%29%7Bwindow._pjscMeta.forceFinish=true;%7D%7D,50%29;%22%5D%7D%7D';
			...
			if($usephantomsimple==true) {	
				...
				if ( is_array( $response ) ) {
				  ...
				  $fileurlcontents = $response['body']; // use the content
					...
					echo esc_html__('Error 03: Error finding reviews. Please contact plugin support.', 'wp-review-slider-pro');
					die();
				}
					...
					//if($vactionrental==true){
						...
						} else if($html->find('div.reviewSelector')){
							$reviewobject = $html->find('div.reviewSelector');
						} else if($html->find('div[class=review-container]')){
							$reviewobject = $html->find('div[class=review-container]');
						} else if($html->find('div._1c8_1ITO', 0)){
							$reviewobject = $html->find('div._1c8_1ITO', 0)->children;
						} else if($html->find("div[class=Dq9MAugU T870kzTX LnVzGwUB]")){
							//another attraction review
							$reviewobject = $html->find("div[class=Dq9MAugU T870kzTX LnVzGwUB]");
						} else if($html->find('div[class=eVykL Gi z cPeBe MD cwpFC]',0)){
							//another attraction review
							//eVykL Gi z cPeBe MD cwpFC
							//echo "here4";
							$reviewobject = $html->find('div[class=eVykL Gi z cPeBe MD cwpFC]');
						} else if($html->find('div.bPhtn', 0)){
							//another attraction review
							$reviewobject = $html->find('div.bPhtn', 0)->children;
						} else if($html->find('div.dHjBB', 0)){
							//another attraction review
							$reviewobject = $html->find('div.dHjBB', 0)->children;
						} else if($html->find("div[class=cWwQK MC R2 Gi z Z BB dXjiy]")){
							//another hotel
							$reviewobject = $html->find("div[class=cWwQK MC R2 Gi z Z BB dXjiy]");
						}  else if($html->find('div.LbPSX', 0)){
							//another attraction review
							$reviewobject = $html->find('div.LbPSX', 0)->children;
						}   else if($html->find('div[class=YibKl MC R2 Gi z Z BB pBbQr]', 0)){
							//another attraction review
							$reviewobject = $html->find('div[class=YibKl MC R2 Gi z Z BB pBbQr]', 0);
						} else {
					...
					if(!isset($reviewobject)){
						echo esc_html__('Error 103a: Unable to read TripAdvisor page. Please use a Review Funnel to grab these reviews.', 'wp-review-slider-pro');
						//echo $html;
						die();	
					}
					...
					foreach ($reviewobject as $review) {
						...
						if($vactionrental==false){
							...
							if($rating>0 && $rtext!=''){
								...
								//echo $user_name.":".$datesubmitted.":".$unixtimestamp.":".$timestamp.":-----</br>";
...
//-----form functions--------------
	...
	public function wprevpro_download_airbnb_master_perurl($currenturl,$urlnum) {
		...
		if(trim($pageid)!=''){
			...
			$this->updatetotalavgreviews('airbnb', trim($pageid), $urldetails['avg'], $totalreviews,trim($pagename));
		}
		//reviews added to db
		if($insertnum>0){
	...
	//==========end old airbnb method, will delete this soon.
	...
	private function getpagename_vrbo($fileurlcontents){
		...
		if($totalrevessearch->item(0)){
			//print_r($totalrevessearch);
			$temptotalreviews = intval($totalrevessearch->item(0)->nodeValue);
		}
		//update the badge total and average here
		$reviewurl['totalreviews'] = $temptotalreviews;
		if($reviewurl['totalreviews']==''){
			$reviewurl['totalreviews'] = $this->get_string_between($fileurlcontents, '"reviewCount": ', '}');
		}
		...
		if($reviewurl['pagetitle']==''){
			$reviewurl['pagetitle'] = $this->get_string_between($fileurlcontents, '"name": "', '",');
		}
		...
		$reviewurl['avgreviews'] = $this->get_string_between($fileurlcontents, '"ratingValue": ', ',');
	...
	public function wprevpro_download_vrbo_master_perurl($currenturl,$urlnum) {
		...
		foreach ($vrbourl as $urlvalue) {
			...
			$data = wp_remote_get( $urlvalue,$args );
			if ( is_wp_error( $data ) ) 
			{
				$response['error_message'] 	= $data->get_error_message();
				$reponse['status'] 		= $data->get_error_code();
...
//print_r($pagedata);
			...
			foreach ($pagedata as $review) {
					...
					if($rating>0){
						...
						if ($wpdb->last_error) {
						  echo 'wpdb_error: ' . $wpdb->last_error;
						}
		...
		if(trim($pageid)!=''){
			$this->updatetotalavgreviews('vrbo', trim($pageid), $urldetails['avgreviews'], $urldetails['totalreviews'], trim($pagename));
		}
		//reviews added to db
		if($insertnum>0){
...
//print_r($temptext);			
	...
	public function wprp_getapps_getrevs_ajax_go($fid,$pagenum=1,$perpage=100,$revsinsertedsofar=0,$nextpageurl='',$iscron='no'){
		...
		$getappsclass = new GetAppsReviews_Functions();
		...
		if($sitetype=='SourceForge'){
			...
			$getreviewsarray= $getappsclass->wprp_getapps_getrevs_page_reviewsio($sitetype,$listedurl,$pagenum,$perpage,$savedpageid,$sortoption,$fid,$blockstoinsert);
		} else if($sitetype=='TripAdvisor'){
			$getreviewsarray= $getappsclass->wprp_getapps_getrevs_page_tripadvisor($sitetype,$listedurl,$pagenum,$perpage,$savedpageid,$sortoption,$fid,$blockstoinsert,$nextpageurl);
			...
			if(is_array($reviewsarray) && count($reviewsarray)>0){
				...
				foreach($reviewsarray as $item) {
					...
					if($sitetype=='FeedbackCompany' || $sitetype=='Reviews.io'){
						...
						$checkrow = $wpdb->get_var( "SELECT id FROM ".$table_name." WHERE reviewer_name = '".$searchname."' AND type = '".$sitetype."' AND (review_length_char = '".$review_length_char."' OR created_time_stamp = '".$unixtimestamp."')" );
					}
	...
	public function wprp_getapps_getrevs_page($type,$listedurl,$pagenum,$perpage,$savedpageid,$nhful,$fid){
		$result['ack']='success';
		//$reviewsarray = array();
		...
		if($type=='StyleSeat'){
			...
			if($pagenum==1){
					...
					$matchavg = $this->get_string_between($fileurlcontents, ',"avg_rating":', ',');
					$matchtot = $this->get_string_between($fileurlcontents, ',"total_review_count":', ',');
					sleep(2);
			}
			...
			//https://pro.experience.com/api/review/david-talbott?profileType=Agent&ratingRange=0,5&sortBy=Newest&offset=0&limit=10&agent_id=162636

			$callurl = $plainurl."?profileType=".$paramarray['profileType']."&tierCategory=".$paramarray['tierCategory']."&ratingRange=0,5&sortBy=Newest&offset=".$tempoffset."&limit=".$limitnum."&agent_id=".$paramarray['agent_id'];
			
			$result['callurl'] =$callurl;

			if (ini_get('allow_url_fopen') == true) {
				$fileurlcontents=file_get_contents($callurl);
			} else if (function_exists('curl_init')) {
				$fileurlcontents=$this->file_get_contents_curl($callurl);
			} else {
				$fileurlcontents='<html><body>'.esc_html__('fopen is not allowed on this host.', 'wp-review-slider-pro').'</body></html>';
				$errormsg = $errormsg . '<p style="color: #A00;">'.esc_html__('fopen is not allowed on this host and cURL did not work either. Ask your web host to turn fopen on or fix cURL.', 'wp-review-slider-pro').'</p>';
				$this->errormsg = $errormsg;
			...
			//$tempjson =  $this->get_string_between($fileurlcontents, '<pre style="word-wrap: break-word; white-space: pre-wrap;">', '</pre>');
			...
			$reviewsarraytemp = $getrevsarray['result'];
			
			//loop reviews and build new array of just what we name
			foreach ($reviewsarraytemp as $item) {
				$tempuserpic = $item['customer_photo_url'];
				$timestamp = date("Y-m-d H:i:s", $item['review_provided_at']);
				$tempname = trim($item['customer_first_name'])." ".trim($item['customer_last_name']);
			...
			$callurl = "https://nextdoor.com/web/feeds/recommendations/?page_id=".$savedpageid;
			$result['callurl'] =$callurl;
			...
			if (filter_var($callurl, FILTER_VALIDATE_URL)) {
				...
				//--------------phantomjscloud-------------------------
					...
					$scriptpayload = "var counter=5;_pjscMeta.manualWait=true;var wprevscrollpageby=setInterval(function(){counter--;window.scrollBy(0,500);if(counter===0){_pjscMeta.manualWait=false;clearInterval(wprevscrollpageby)}},300)";
					...
					if ($html === FALSE) { 
						...
						//$startofjsonnum = strpos($html,'<script type="application/ld+json">') + 50;
						//$jsonhtml = substr($html,$startofjsonnum);
						//$emdjsonnum = strpos($jsonhtml,'</script>');
						//$schemajson = substr($jsonhtml,0,$emdjsonnum);
						
						$schemajson =$this->get_string_between($html, '<script type="application/ld+json">', '</script>');
					...
					$startofjsonnum = strpos($html,'},"reviews":[') + 12;
					//echo "start:".$startofjsonnum;
					
						$html = substr($html,$startofjsonnum);
						//echo $html;
						$emdjsonnum = strpos($html,'],"offers":') + 1;
...
//die();
	...
	//specialiazed phantomjscall for google
	public function phantomjscloudcall($callurl,$popupreviews='',$nhful='',$placetype='',$proxy='',$apikey='ak-2cme5-eqftq-dv73x-nr41t-gkbvs' ){
			
		//testing--
		$requestdetails = '{url:"'.$callurl.'",renderType:"html"}';
		//$request = urlencode($requestdetails);
		$tempurlvalue = 'https://phantomjscloud.com/api/browser/v2/ak-2cme5-eqftq-dv73x-nr41t-gkbvs/?request='.$requestdetails;
		$results['tempurlvalue']=$tempurlvalue;
		//-----
		$proxytext='';
		if($proxy!=''){
			$proxytext = '"proxy":"'.$proxy.'",';
		}
			...
			if($placetype=='hotel'){
				$payload = '{
								"url":"'.$callurl.'",
								"renderType":"html",
								"outputAsJson":false,'.$proxytext.'
								"overseerScript":"await page.waitForNavigation();var elementcheck = await page.evaluate(()=>{ return document.querySelector(\"div[class=NDCC7b] a\") });if(elementcheck!=null){await page.click(\"div[class=NDCC7b] a\");}page.render.content();"
							}';
				...
				if($popupreviews=='yes'){
					if($nhful=='newest'){
						$payload = '{
							...
							"renderType":"html",
							"outputAsJson":false,'.$proxytext.'
							"overseerScript":"await page.waitForNavigation();await page.click(\"a[data-async-trigger=reviewDialog]\");await page.waitForDelay(4000);await page.click(\"div[data-sort-id=newestFirst]\");page.render.content();"
						}';
					} else {
						$payload = '{
								"url":"'.$callurl.'",
								"renderType":"html",
								"outputAsJson":false,'.$proxytext.'
								"overseerScript":"await page.waitForNavigation();await page.click(\"a[data-async-trigger=reviewDialog]\");page.render.content();"
							}';
					}
				} else {
					if($nhful=='newest'){
						$payload = '{
							...
							"renderType":"html",
							"outputAsJson":false,'.$proxytext.'
							scripts:{"loadFinished": [
										"const sortbuttons = document.querySelectorAll(\'div[data-sort-id]\');sortbuttons[1].click();",
										]}
						}';
					} else {
						$payload = '{
							...
							"renderType":"html",
							"outputAsJson":false,'.$proxytext.'
							"overseerScript":"await page.waitForNavigation({waitUntil:\'domcontentloaded\'});page.render.content();"
						}';
					}
				}
			}
			...
			//die();
				...
				$results['fileurlcontents'] = $response['body'];
	...
	public function wprevpro_ajax_crawl_placeid(){
		...
		$tempurlvalue = 'https://crawl.ljapps.com/crawlrevs?rip='.$ip_server.'&surl='.$siteurl.'&stype=googlecheck&sfp=pro&scrapequery='.urlencode($gplaceid);
