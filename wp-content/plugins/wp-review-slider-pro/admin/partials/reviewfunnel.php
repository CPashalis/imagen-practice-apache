 ...
 */
 ...
 //global variables for using freemius api
	...
	if (isset($_POST['wprevpro_submittemplatebtn'])){
		...
		if($t_id==""){
			//must be inserting.
			if($this->intpluginversion > 1148){
				$reviewlistpageid = str_replace(" ","",$title);
				$reviewlistpageid = str_replace("'","",$reviewlistpageid);
				$reviewlistpageid = str_replace('"',"",$reviewlistpageid);
				$reviewlistpageid = preg_replace('/[^A-Za-z0-9\-]/', '', $reviewlistpageid)."_rf".$site_type;
			}
			$data = array( 
				'title' => "$title",
			...
			//update
			$editpluginversion = sanitize_text_field($_POST['editpluginversion']);
			if($editpluginversion > 1148){
				$reviewlistpageid = str_replace(" ","",$title);
				$reviewlistpageid = str_replace("'","",$reviewlistpageid);
				$reviewlistpageid = str_replace('"',"",$reviewlistpageid);
				$reviewlistpageid = preg_replace('/[^A-Za-z0-9\-]/', '', $reviewlistpageid)."_rf".$site_type;
